...
class Baichuan_base_generator(model_generator):
    def __init__(self, f_path, is_few_shot, device, is_vllm, model_path, model_name, tensor_parallel_size, gpu_memory_utilization):
        ...
        '''
        super(Baichuan_base_generator, self).__init__(f_path, is_few_shot, device, is_vllm, model_path, tensor_parallel_size, gpu_memory_utilization)
        ...
        for instruction, input_text, answer in tqdm(zip(instruction_ls, input_text_ls, answer_ls),
            ...
            try:
                ...
                if q_type == "multiple_choice":
                    ...
                    response = match.group(1) if match else "未成功回答"
            except Exception as e:
                num_unsuccess += 1
                print(f"Fail to answer: {e}")
                response = "未成功回答"

            output_ls.append({"input": input_text, "output": response, "answer": answer})
