...
class Internlm_base_generator(model_generator):
    def __init__(self, f_path, is_few_shot, device, is_vllm, model_path, model_name,tensor_parallel_size, gpu_memory_utilization):
        ...
        '''
        super(Internlm_base_generator, self).__init__(f_path, is_few_shot, device, is_vllm, model_path, tensor_parallel_size, gpu_memory_utilization)
            ...
            for instruction, input_text, answer in tqdm(zip(instruction_ls, input_text_ls, answer_ls),
                ...
                try:
                    output = model.generate(
                        **inputs,
                        do_sample=False,
                        max_new_tokens=20) if q_type == "multiple_choice" else model.generate(**inputs, max_new_tokens=512, do_sample=False)
